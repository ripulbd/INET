                ...
                if (natVector.size()>0)
                {
                    for (NatVector::iterator it=natVector.begin(); it!=natVector.end(); it++)
                    {
                        if ((*it)->peer1 == nat->getPeer1() || (*it)->peer1Assoc == assocId)
                        {
                            ...
                            if ((*it)->peer1 == 0 && (*it)->peer1Address2 != Address("0.0.0.0")) {
                            ...
                            if (nat->getMulti() && (*it)->peer1Address2==Address("0.0.0.0"))
                            {
                                (*it)->peer1Address2 = ind->getRemoteAddr();
                            }
                            if ((*it)->peer2Address1!=Address("0.0.0.0") && (*it)->peer1Address1!=Address("0.0.0.0"))
                            {

                                if (!(*it)->multi || ((*it)->multi && (*it)->peer2Address2!=Address("0.0.0.0") && (*it)->peer1Address2!=Address("0.0.0.0")))
                                {
                        ...
                        if ((*it)->peer2 == nat->getPeer1() || (*it)->peer2Assoc == assocId)
                        {
                            ...
                            if ((*it)->peer2Address1==Address("0.0.0.0"))
                            {
                            ...
                            else if ((*it)->multi && (*it)->peer2Address2 != Address("0.0.0.0"))
                                (*it)->peer2Address2 = ind->getRemoteAddr();

                            if (!(*it)->multi || ((*it)->multi && (*it)->peer2Address2!=Address("0.0.0.0") && (*it)->peer1Address2!=Address("0.0.0.0") 
                                && (*it)->peer2Address1!=Address("0.0.0.0") && (*it)->peer1Address1!=Address("0.0.0.0")))
                            {
                ...
                if (natVector.size()>0)
                {
                    for (NatVector::iterator it=natVector.begin(); it!=natVector.end(); it++)
                    {
                        if ((*it)->peer1Assoc==assocId)
                        {
                            ...
                            if ((*it)->multi && (*it)->peer1Address2==Address("0.0.0.0"))
                            {
                                (*it)->peer1Address2 = ind->getRemoteAddr();
                    ...
                    sctpEV3 << info->peer1 << " and assoc " << assocId << "\n";;
                    info->multi = 1;
                    info->peer1Address1 = Address("0.0.0.0");
