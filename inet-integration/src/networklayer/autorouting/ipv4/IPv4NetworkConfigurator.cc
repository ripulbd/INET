...
void IPv4NetworkConfigurator::configureRoutingTable(IIPv4RoutingTable *routingTable)
{
    ...
    for (int i = 0; i < topology.getNumNodes(); i++) {
        Node *node = (Node *)topology.getNode(i);
        //std::cout << "Configuring routing table for Node:" << node->getModule()->getFullPath() << endl;
...
void IPv4NetworkConfigurator::extractTopology(IPv4Topology& topology)
{
    ...
    std::cout << "Topology found at IPv4Network " << topology.getNumNodes() << " nodes\n";
    ...
    for (int linkIndex = 0; linkIndex < (int)topology.linkInfos.size(); linkIndex++)
    {
        ...
        std::cout<< "LinkInfo:"<< linkInfo->detailedInfo().c_str() << endl;
...
IPv4NetworkConfigurator::InterfaceInfo *IPv4NetworkConfigurator::determineGatewayForLink(LinkInfo *linkInfo)
{
    ...
    for (int interfaceIndex = 0; interfaceIndex < (int)linkInfo->interfaceInfos.size(); interfaceIndex++)
    {
        ...
        hostName = interfaceInfo->node->module->getFullPath();

        //std::cout<< interfaceInfo->node->module->getFullPath() << endl;

        if (hostName.find(host) != std::string::npos) {
            //std::cout << "Host found!" << endl;
            found = true;
        }
        ...
        if (numInterfaces > 1 && routingTable && routingTable->isForwardingEnabled())
        {
            ...
            {
                if(found) {
                    std::cout<< "Host IP Address::" << interfaceInfo->getAddress() << endl;
                }
                else gatewayInterfaceInfo = interfaceInfo; // remember gateway
            }
        ...
        uint32& mergedNetmask, uint32& mergedNetmaskSpecifiedBits, uint32& mergedNetmaskIncompatibleBits)
{
    ...
    for (int interfaceIndex = 0; interfaceIndex < (int)interfaces.size(); interfaceIndex++)
    {
        ...
        //std::cout << "Trying to merge " << ie->getFullPath() << " interface with address specification: " << IPv4Address(candidateAddress) << " / " << IPv4Address(candidateAddressSpecifiedBits) << endl;
        //std::cout << "Trying to merge " << ie->getFullPath() << " interface with netmask specification: " << IPv4Address(candidateNetmask) << " / " << IPv4Address(candidateNetmaskSpecifiedBits) << endl;
...
void IPv4NetworkConfigurator::assignAddresses(IPv4Topology& topology)
{
    ...
    for (int linkIndex = 0; linkIndex < (int)topology.linkInfos.size(); linkIndex++)
    {
        ...
        //std::cout << "Selected Link:" << selectedLink->getFullPath().c_str() << endl;
...
void IPv4NetworkConfigurator::readInterfaceConfiguration(IPv4Topology& topology)
{
    ...
    for (int i = 0; i < (int)interfaceElements.size(); i++)
    {
        ...
        try
        {
            ...
            //std::cout << "Fixing for hosts:" << hostAttr << endl;


            // configure address/netmask constraintDefaultRoutes on matching interfaces
            for (int i = 0; i < (int)topology.linkInfos.size(); i++)
            {
                ...
                for (int j = 0; j < (int)linkInfo->interfaceInfos.size(); j++)
                {
                    ...
                    if (interfacesSeen.count(interfaceInfo) == 0)
                    {
                        ...
                        std::string hostFullPath = hostModule->getFullPath();
                        //std::cout << "hostFullPath:" << hostFullPath << endl;
                        std::string hostShortenedFullPath = hostFullPath.substr(hostFullPath.find('.') + 1);
                            ...
                            (towardsMatcher.matchesAny() || linkContainsMatchingHostExcept(linkInfo, &towardsMatcher, hostModule)))
                        {
                            ...
                            //std::cout << hostModule->getFullPath() << ":" << interfaceInfo->interfaceEntry->getFullName() << endl;
                        }
                    }
                }
            }
...
void IPv4NetworkConfigurator::addStaticRoutes(IPv4Topology& topology)
{
    ...
    for (int i = 0; i < topology.getNumNodes(); i++) {
        ...
        if (addDefaultRoutesParameter && sourceNode->interfaceInfos.size() == 1 && sourceNode->interfaceInfos[0]->linkInfo->gatewayInterfaceInfo)
        {
          if (sourceNode->interfaceInfos[0]->addDefaultRoute)
          {
            ...
            std::cout<< "Source node:" << sourceNode->interfaceInfos[0]->getAddress() << " and its gateway:" << gateway << endl;
        ...
        else
        {
            ...
            for (int j = 0; j < topology.getNumNodes(); j++)
            {
                ...
                if (link->destinationInterfaceInfo && link->destinationInterfaceInfo->addStaticRoute)
                {
                    ...
                    // add the same routes for asetDestinationll destination interfaces (IP packets are accepted from any interface at the destination)
    ...
    for (int j = 0; j < topology.getNumNodes(); j++) {
        // extract destination
        Node *node = (Node *) topology.getNode(j);

        //it works....
        //std::cout << "Node:" << j << ": " << node->getModule()->getFullName() << " - " << node->getModule()->getFullPath() << endl;
        std::string nodeName = node->getModule()->getFullName();
        std::vector<InterfaceInfo *> interfaceInfos = node->interfaceInfos;
        if(nodeName.compare("natRouter") == 0){
            std::cout<< "I've found a natRouter!!" << endl;
            for(int i = 0; i < interfaceInfos.size(); i++){
                InterfaceInfo *info = interfaceInfos[i];

                std::string address = info->getAddress().str();
                std::cout<< "Interface-" << info->interfaceEntry->getFullName() << " with address:" << address << endl;
                if (address.find("100") != std::string::npos) {
                    //std::cout << "Address to Rotuer found!" << '\n';
                    unsigned found = address.find_last_of(".");
                    std::string first = address.substr(0, found);
                    std::string last = address.substr(found + 1);

                    int lastPart;
                    if (!(std::istringstream(last) >> lastPart))
                        lastPart = 0;
                    lastPart--;
                    std::ostringstream convert;
                    convert << lastPart;
                    std::string newAddress = first + "." + convert.str();

                    const char *addressChar = newAddress.c_str();
                    IPv4Address *destAddress = new IPv4Address(addressChar);

                    IPv4Route *route = new IPv4Route();
                    //route->setDestination(IPv4Address::UNSPECIFIED_ADDRESS);
                    IPv4Address *newDest = new IPv4Address();
                    route->setDestination(IPv4Address());
                    route->setGateway(*destAddress);
                    route->setNetmask(info->getNetmask());
                    route->setInterface(info->interfaceEntry);
                    route->setSourceType(IPv4Route::MANUAL);

                    //node->routingTable->addRoute(route, true);
                    node->routingTable->addRoute(route);
                    node->staticRoutes.push_back(route);
                    std::cout << "New Address for the router interface: " << *destAddress << '\n';

                    node->routingTable->printRoutingTable();

                } else if (address.find("10") != std::string::npos) {
                    unsigned found = address.find_last_of(".");
                    std::string first = address.substr(0,found);
                    std::string last = address.substr(found+1);
                    //int lastPart = std::atoi(last.) + 1;

                    int lastPart;
                    if ( ! (std::istringstream(last) >> lastPart) ) lastPart = 0;
                    lastPart++;
                    std::ostringstream convert;
                    convert << lastPart;
                    std::string newAddress = first + "." + convert.str();
                    ...
                    std::cout << "New Address: " << newAddress << '\n';
                }
            }

        }
        if(nodeName.compare("router[0]") == 0){
            std::cout<< "I've found a router!!" << endl;
        }
    }
}
