...
void IPv4RoutingTable::printRoutingTable() const
{
    ...
    std::cout << stringf("%-16s %-16s %-16s %-4s %-16s %s\n", "Destination",
                      "Netmask", "Gateway", "Iface", "", "Metric");

    for (int i = 0; i < getNumRoutes(); i++) {
        IPv4Route *route = getRoute(i);
        InterfaceEntry *interfacePtr = route->getInterface();
        std::cout << stringf("%-16s %-16s %-16s %-4s (%s) %d\n",
...
void IPv4RoutingTable::internalAddRoute(IPv4Route *entry)
{
    ...
    //std::cout << "Destination address:" << entry->getDestination().str().c_str() << endl;
    EV_DEBUG << "Destination address:" << entry->getDestination().str().c_str() << endl;
...
void IPv4RoutingTable::addRoute(IPv4Route *entry)
{
    ...
    std::cout << entry->getNetmask().getInt() << ":" << entry->getDestination().getInt() << ":" << entry->getNetmask().getInt() << endl;
    if (entry->getNetmask().getInt() != 0 && (entry->getDestination().getInt() & entry->getNetmask().getInt()) == 0)return;
...
void IPv4RoutingTable::addRoute(IPv4Route *entry, bool flag)
{
    Enter_Method("addRoute(...)");
    //if (entry->getNetmask().getInt() != 0 && (entry->getDestination().getInt() & entry->getNetmask().getInt()) == 0)return;
    internalAddRoute(entry);
